// boozoRpc — 24/7 Background RPC & Status Daemon
...
export class RpcDaemon {
  ...
  public async start(): Promise<void> {
    ...
    console.log('[boozoRpc Daemon] Starting 24/7 background daemon...')

    // Run initial sync
    await this.syncAllUsers().catch(err => {
      console.error('[boozoRpc Daemon] Initial sync error:', err)
    })
    ...
    this.tickTimer = setInterval(async () => {
      try {
        ...
        console.error('[boozoRpc Daemon] Tick error:', err)
      }
    }, 30000)

    console.log('[boozoRpc Daemon] 24/7 daemon started successfully.')
  }
  ...
  public stop(): void {
    ...
    this.sockets.clear()
    console.log('[boozoRpc Daemon] Daemon stopped.')
  }
    ...
    for (const [userId, userSock] of this.sockets.entries()) {
      try {
        ...
        if (!session.user.trial || !session.user.trial.active || session.user.trial.endsAt < now) {
          console.log(`[boozoRpc Daemon] Trial expired for user ${userId}, disabling presence`)
        ...
        if (session.sleepTimerActive && session.sleepTimerEndsAt && session.sleepTimerEndsAt <= now) {
          console.log(`[boozoRpc Daemon] Sleep timer elapsed for user ${userId}`)
          await db.session.update({
            where: { id: session.id },
            data: {
        ...
        if (session.discordTokenExpiresAt && session.discordTokenExpiresAt.getTime() - now.getTime() < 30 * 60 * 1000) {
          if (session.discordRefreshToken) {
            console.log(`[boozoRpc Daemon] Refreshing Discord token for user ${userId}...`)
        ...
        console.error(`[boozoRpc Daemon] Error in tick for user ${userId}:`, userTickErr)
      }
    }
  }
  ...
  public async stopUserRpc(userId: string): Promise<void> {
    ...
    if (userSock && userSock.ws && userSock.ws.readyState === WebSocket.OPEN) {
      ...
      try {
        ...
        console.error(`[boozoRpc Daemon] Error sending clear OP 3 for user ${userId}:`, err)
      }
  ...
  private async connectUserSocket(userId: string): Promise<void> {
    ...
    try {
      ...
      ws.on('message', async (data: Buffer | string) => {
        try {
          ...
          if (op === 10) {
            ...
            userSock.heartbeatTimer = setInterval(() => {
              if (ws.readyState === WebSocket.OPEN) {
                if (!userSock.heartbeatAck) {
                  console.warn(`[boozoRpc Daemon] Zombie socket detected for user ${userId} (missing ACK). Reconnecting...`)
                  this.cleanupSocket(userSock)
            ...
            console.log(`[boozoRpc Daemon] Discord Gateway sent OP 7 RECONNECT for user ${userId}. Reconnecting...`)
            this.cleanupSocket(userSock)
            this.scheduleReconnect(userId, 1000)
          } else if (op === 9) {
            // Invalid session
            console.warn(`[boozoRpc Daemon] Discord Gateway sent OP 9 INVALID_SESSION for user ${userId}`)
            this.cleanupSocket(userSock)
            this.scheduleReconnect(userId, 3000)
          }
        } catch (err) {
          console.error(`[boozoRpc Daemon] Error handling WS message for user ${userId}:`, err)
        }
      })

      ws.on('error', (err) => {
        console.error(`[boozoRpc Daemon] Gateway WS error for user ${userId}:`, err.message)
      ...
      ws.on('close', async (code, reason) => {
        console.log(`[boozoRpc Daemon] Gateway WS closed for user ${userId} (code: ${code}, reason: ${reason.toString() || 'none'})`)
        userSock.connected = false
        userSock.isConnecting = false
        this.cleanupSocket(userSock)
        ...
        if (code === 4004) {
          ...
          console.log(`[boozoRpc Daemon] Auth failed (4004) for user ${userId}. Refreshing token...`)
          if (session.discordRefreshToken) {
            const refreshed = await refreshDiscordToken(session.discordRefreshToken)
            if (refreshed) {
              await db.session.update({
          ...
          console.warn(`[boozoRpc Daemon] Rate limited by Discord Gateway for user ${userId}. Backing off for 60s...`)
      ...
      console.error(`[boozoRpc Daemon] Failed to initialize connection for user ${userId}:`, err)
      userSock.connected = false
      userSock.isConnecting = false
      this.scheduleReconnect(userId)
    }
  ...
  private async pushPresenceForUser(userId: string, session: any, force: boolean = false): Promise<void> {
    ...
    try {
      ...
      console.warn(`[boozoRpc Daemon] Transient DB error in pushPresenceForUser for user ${userId}:`, dbErr)
      return
    }
    ...
    try {
      console.log(`[boozoRpc Daemon] Sending OP 3 for user ${userId}: status=${status}, activities=${JSON.stringify(activities)}`)
      userSock.ws.send(JSON.stringify({
        op: 3,
        d: {
          status,
      ...
      console.error(`[boozoRpc Daemon] Failed to send OP 3 for user ${userId}:`, err)
    }
  }
  ...
  public disconnectUser(userId: string): void {
    ...
    this.cleanupSocket(userSock)
    this.sockets.delete(userId)
    console.log(`[boozoRpc Daemon] Disconnected user ${userId}`)
  }
...
export function ensureDaemonRunning(): RpcDaemon {
  ...
  daemon.start().catch(err => {
    console.error('[boozoRpc Daemon] Failed to start daemon:', err)
  })
  return daemon
}
